...
lambda = fmincon(@(l)damage_calibS(l),0.35,[],[],[],[],0.00001,0.999999,[],optimset('Tolfun',1e-12));
...
if mode == 1
    ...
    t0=taue3rho15taxonly; % initial guess for the input tax
    x0=t0;
...
if display_iter==1
    if display_diag==1
    options = optimset('Display','iter', 'Diagnostics','on','FunValCheck','on','TolFun',1e-12,'TolX',1e-10, 'MaxFunEvals', 100000);
    else
    options = optimset('Display','iter', 'Diagnostics','off', 'FunValCheck','on','TolFun',1e-12,'TolX',1e-10, 'MaxFunEvals', 100000);
    end
else
    if display_diag==1
    options = optimset('Display','off', 'Diagnostics','on','FunValCheck','on','TolFun',1e-12,'TolX',1e-10, 'MaxFunEvals', 100000);
    else 
    options = optimset('Display','off', 'Diagnostics','off', 'FunValCheck','on','TolFun',1e-12,'TolX',1e-10, 'MaxFunEvals', 100000);
    end
end    
if mode == 1
...
if exitflag > 0
    disp('Optimization converged to a solution.');
elseif exitflag == 0
    disp('Maximum number of iterations or function evaluations exceeded.');
elseif exitflag == -1
    disp('Optimization terminated by the output function.');
elseif exitflag == -2
    disp('No feasible solution found.');
elseif exitflag == -3
    disp('Problem appears to be unbounded.');
elseif exitflag == -4
    disp('NaN or Inf value encountered during optimization.');
else
    disp('Unknown exitflag value.');
end


%********************************************************************
%********************************************************************
%7. Results extraction: Util is calculated using the results from the
%optimization, Acc; Add... store the path of the variables
